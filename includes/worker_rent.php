...
function jiwu_process_rent_tasks() {

    error_log('start jiwu process rent task.');
    // 设置无限执行时间
    set_time_limit(0);
    ...
    // 定义队列名称
    $queue = 'jiwu_rent_task_queue';

    // 获取全局 $wpdb 对象
    global $wpdb;
    $table = $wpdb->prefix . 'rent_listings';
...
//            error_log(print_r($data, true));
            ...
            // 处理任务
            $rst = process_rent_property($data);
...
function process_rent_property($listing): bool
{
    global $wpdb;
    // 构造自定义表名（考虑表前缀）
    $table_name = $wpdb->prefix . 'rent_listings';
    // 校验记录必要字段是否合法
    if (!rent_validate_listing($listing)) {
    ...
    $imported_ref_key = '_imported_rent_ref_jiwu';
    ...
    if ($inserted_updated !== false) {
        ...
        if ($listing->price) {
            // 设置价格
            update_post_meta($post_id, 'fave_property_price', $listing->price);
        }
        ...
        if ($listing->bond) {
            update_post_meta($post_id, 'fave_property-bond', $listing->bond);
        }

        if ($listing->bond_text) {
            update_post_meta($post_id, 'fave_property-bond-text', $listing->bond_text);
        }

        if ($listing->available_date) {
            update_post_meta($post_id, 'fave_property-available', $listing->available_date);
        }
